...
const About = () => {
  ...
  const EditCard = (id) =>{
      index = id;
      setpostId(allData[index].id)
      setPTitle(allData[index].title)
      setPBody(allData[index].body)
  }
  const CardUpdate = (id) =>{
    let update = allData[index]
    update.id = pId
    update.title = pTitle
    update.body = pBody
    localStorage.setItem("value", JSON.stringify(allData));
  ...
  return (
    <>
      ...
      <button to="About" onClick={openModal} className=" decoration-sky-500 text-blue-500 font-bold ... text-6xl float-right  mt-11"><Icons.FaPlusCircle /></button>
            ...
            >
              <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                <div className="mt-4">
                  ...
                  <button className='inline-flex justify-center px-4 py-2 text-sm font-medium text-blue-900 bg-blue-100 border border-transparent rounded-md hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500' type="button" onClick={CardUpdate}>Update</button>
                </div>
              </div>
            </Transition.Child>
          </div>
        ...
        {showData[common]?.post?.map((val, i) => (
          <div className="postContent" class="p-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 xl:grid-cols-3 ml-9">
            <div className="postData">
              ...
              <input type="text" value={val.id} /><br/>
              {localStorage.setItem("value", JSON.stringify(allData))}
              <strong><label className="mr-2">Title:-</label></strong>
              {/* <EditText defaultValue={val.title} inline /><br /> */}
              <input type="text" value={val.title} /><br/>
              {localStorage.setItem("value", JSON.stringify(allData))}
              <strong><label className="mr-2">Body:-</label></strong><br/>
              {/* <EditTextarea defaultValue={val.body} rows={8} /> */}
              <textarea type="text" value={val.body} rows={10} />
              {localStorage.setItem("value", JSON.stringify(allData))}
              ...
              <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" type="button" onClick={() => EditCard(i)}>Edit</button>
            </div>
          </div>
        ))}
      </div>
